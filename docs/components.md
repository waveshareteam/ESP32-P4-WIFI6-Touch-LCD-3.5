...
| --- | --- | --- |
| ESP32-P4-WIFI6-Touch-LCD-3.5 BSP | Product examples | Managed dependency `waveshare/esp32_p4_wifi6_touch_lcd_3_5 ==2.0.2` |
...
The Registry package was audited for the migration; product manifests and
public dependency references use the published package and version above.

Version 2.0.2 is published in Component Registry and is pinned by every product
manifest that declares this BSP. A merged component or BSP change is not a
product dependency until the matching Registry release is publicly available.
Do not replace the version with a Git URL, branch, commit, local path, or an
unpublished version: those forms are unsuitable for a publishable product
manifest and may be rejected by Component Registry CI.
...
The official BSP 2.0.2 public header fixes the display to RGB565,
...
made `bsp/display.h` self-contained by including the definition of `esp_err_t`.
After the resulting 2.0.2 package was published and validated, the two
screen-source includes that existed only as a product-side workaround were
removed. Files that directly use ESP error types still include `esp_err.h`.
...
4. Keep Example 12 on `lvgl/lvgl 8.3.*` while its 21 SquareLine-generated image
   ...
   BSP 2.0.2 exposes the matching LVGL 8 rotation type directly, so the former
...
7. The complete LCD-5 Arduino library baseline is source bundled under
   `examples/arduino/`; the LCD-3.5 board layer remains the active hardware
   adapter. These Arduino sources do not change or shadow the managed BSP
   dependency used by ESP-IDF examples.
