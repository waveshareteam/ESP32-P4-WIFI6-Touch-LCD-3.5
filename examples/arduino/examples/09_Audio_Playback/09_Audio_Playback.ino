/*
 * ES8311 melody player for the Waveshare ESP32-P4-WIFI6-Touch-LCD-3.5.
 *
...
esp_err_t es8311_codec_init(void) {
  ...
  ESP_RETURN_ON_ERROR(es8311_sample_frequency_config(
                          es_handle, es_clk.mclk_frequency, es_clk.sample_frequency),
                      "ES8311", "sample-rate configuration failed");
  ESP_RETURN_ON_ERROR(es8311_microphone_config(es_handle, false),
                      "ES8311", "analog input configuration failed");
  ESP_RETURN_ON_ERROR(es8311_voice_volume_set(es_handle, MELODY_VOLUME, NULL),
                      "ES8311", "volume configuration failed");
  return ESP_OK;
}
...
void setup() {
  ...
  pinMode(lcd35::kPowerAmplifier, OUTPUT);
  digitalWrite(lcd35::kPowerAmplifier, LOW);
  ...
  i2s_pin_config_t pin_config = {
    ...
    .data_out_num = lcd35::kI2sDataOut,
    .data_in_num = -1,
  };
  ...
  digitalWrite(lcd35::kPowerAmplifier, HIGH);
  audio_ready = true;
