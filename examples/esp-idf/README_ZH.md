...
标准 CI 矩阵为 12 个工程 × ESP-IDF v5.5.5/v6.0.2 = 24 次构建，且全部使用 `rev3_x`
（rev3.x）配置；不会按芯片版本将每个示例矩阵翻倍。维护的产品源码 `12_esp32-p4-eye`
在 IDF v6.0.2 上具有独立的 `rev1_3` 和 `rev3_x` 产品任务/制品。不同配置使用独立
sdkconfig/构建目录，二进制互不兼容。新建本地配置同样默认使用 `rev3_x`；仅已确认 rev1.x
芯片时才显式选择 `rev1_3`。当前没有 Arduino 示例或 Arduino CI 构建；未来默认
Arduino 策略为 `ChipVariant=postv3`。编译通过不是硬件在环或外设验证。
