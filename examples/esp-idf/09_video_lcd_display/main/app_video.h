...
/* Official BSP 2.0.0 fixes the display contract to RGB565. */
 ...
 * Call device lifecycle APIs from one control task; frame delivery is handled by
 * the internally synchronized stream task.
 *
 ...
 * Releases any partial buffer setup on failure and leaves the caller-owned
 ...
 *
 * Requests asynchronous task shutdown. Call app_video_wait_video_stop() before
 * reconfiguring buffers or closing the device.
 *
 * @param video_fd File descriptor for the video device.
...
/**
 * @brief Wait until the video stream task has fully released the stream.
 *
 * @return ESP_OK on success, or an error if the lifecycle semaphore is absent.
 */
